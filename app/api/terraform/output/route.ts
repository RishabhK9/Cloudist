...
export async function POST(request: NextRequest) {
  try {
    ...
    if (credentials?.aws) {
      ...
      console.log('🔑 Using AWS credentials for terraform output')
    } else {
      console.warn('⚠️ No AWS credentials provided for terraform output')
    }

    const result = await executeTerraformCommand('output', ['-json'], workingDirectory, env)
